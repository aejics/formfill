...
<?php
    ...
    if (!$loggedin && $action !== "loginform" && $action !== "login"){
        header('Location: /dblogin.php?action=loginform');
    }
    if ($action == "loginform"){
        print("<div class='h-100 d-flex align-items-center justify-content-center flex-column'>
            ...
            <form action='/dblogin.php?action=login' method='POST' class='w-200' style='max-width: 600px;'>
                <div class='mb-3'>
    ...
    if ($action == "login"){
        ...
        if (strpos($giae->getConfInfo(), 'Erro do Servidor') !== false){
            ...
            $valordb->bindValue(':1', $_POST["user"], SQLITE3_TEXT);
            $valordb->bindValue(':2', $config['nomeutilizador'], SQLITE3_TEXT);
            $valordb->bindValue(':3', $perfil['perfil']['nome'], SQLITE3_TEXT);
            $valordb->bindValue(':4', $perfil['perfil']['email'], SQLITE3_TEXT);
            $valordb->execute();
            header('Location: /');
        }
    };
    if ($loggedin){
        ...
        if (str_contains($giae->getConfInfo(), 'Erro do Servidor')){
            header('Location: /dblogin.php?action=logout');
